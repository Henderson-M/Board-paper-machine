...
| `fetch_pdf_text.py` | Download a document and extract its text. Use when an org publishes board dates inside an annual calendar PDF rather than on a webpage. Try `requests` mode first; pass `--playwright` if the host blocks direct downloads. **Sniffs the file header rather than trusting the URL** — extension-less handler URLs often serve Word, not PDF (Surrey and Sussex publishes its whole pack as `.docx`), and those are extracted as OOXML. Writes UTF-8 to stdout regardless of console codepage. |
...
7b. **A pack published as MANY separate files is the normal case, not the exception — and date-matching under-reads it badly.** Plenty of trusts publish each paper as its own file (`PB-26-169 Integrated Board Report.pdf`, `8.5 Provider Capability Assessment.pdf`), and most of those filenames do NOT carry the meeting date. A detector that keeps only files whose name or row matches `{date}` will therefore capture the two or three that happen to be dated and silently drop the rest — the meeting still looks "detected", the summary still looks complete, and nothing anywhere reports a gap.

   So whenever the papers page has a **dated per-meeting sub-page or section** for `{date}` (e.g. `/board-meeting-24-september-2026/`, `/annual-general-meeting-…-24-september-2026/`), take **every** document linked from that page, regardless of whether each individual filename carries the date. Match the date once, at the level of the sub-page or section heading — then stop filtering below it.

   Sanity-check the count before moving on: a full public board pack is rarely one or two documents. If a trust that normally publishes 20+ papers returns 2, that is a detection failure, not a thin pack.

   Worked failure (2026-09-24): East of England Community Health and Care (RY3) returned **3 of 18** files and Yorkshire Ambulance (RX8) **2 of 37**, both because the per-paper filenames are undated. Both were only recovered because the analyser was separately told to go looking — which is luck, not design. What it nearly cost: RY3's two predecessor annual reports disclose £160,000 in redundancy payments to a named former finance director, and RX8's annual accounts disclose 13 exit packages totalling £732k.

   **Pass the papers URL through from the org record — never hand-type one into an analyser's prompt.** On the same run two analyser prompts were given hand-typed URLs (RY3, RX9); both were wrong, both 404'd, and both agents burned time rediscovering the correct live page. The org record is the maintained field (Step 7.1) — read it and pass it.
...
**Never record `fail` from a single fetch. Escalate the whole ladder first.** A 403 or an empty result from the cheap fetcher is not evidence about the org — it is evidence about that one attempt. On 2026-08-24 three orgs (RXA, RKB, RWR) were recorded as `blocked` off one 403 each; all three read perfectly under Playwright and simply publish no forward schedule, which is `ok --kind no_schedule_published`. Cheshire and Wirral had been carrying a false `broken` status for six runs on that basis. If you are about to record a failure and have not tried Playwright and `--follow-landing`, you do not yet know enough to record it — run:

```bash
python org_urls.py probe --ods RXA --write     # full ladder, then record what it actually found
```

Orgs whose host reliably blocks the cheap fetcher carry `"fetch_mode": "playwright"` in their record. Pass `--ods CODE` to `extract_board_html.py` and it will honour that hint and go straight to the browser instead of spending a doomed request. Set the hint when you discover a new one.
...
- **A multi-file pack is under-read by date-matching alone (Step 7b).** When a trust publishes each paper separately, most filenames carry no meeting date, so a date filter keeps two or three and drops the rest while the meeting still looks fully detected. Match the date once at the per-meeting sub-page, then take everything on it, and treat a 2-document return from a trust that usually publishes 20+ as a failure to investigate. RY3 returned 3 of 18 and RX8 2 of 37 on 2026-09-24.
